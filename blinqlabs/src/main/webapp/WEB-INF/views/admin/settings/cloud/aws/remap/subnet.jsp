									...
									<div>
							            	<section>
												...
												<div class="text-center text-primary">
													...
													<c:forEach var="subnet" items="${subnets}">
														<c:if test="${subnet.ID_COMPONENT==subnetComponent.ID_COMPONENT}">
															<h5>The subnet <span class="text-secondary">(${subnet.TX_COMPONENT_KEY})</span> appears in the following rules. Please choose new subnets to remap the existing rules.</h5>														
															 <c:set var="vpc" value="${subnet.ID_PARENT}"/>
														</c:if>
													</c:forEach>
									  ...
									  <div class="m-2">
										  <div class="row">
											  ...
											  <tbody>
											  		<c:forEach var="rule" items="${subnetRules}" varStatus="loop">
													  	<tr>
												 			<td>${loop.index+1}</td>
									  ...
									  <hr/>
									  <form class="form-horizontal m-t-20" id="remapRuleForm" action="${contextPath}/admin/settings/cloud/aws/remapSubnet" method="POST" name="RulesForm" modelAttribute="rulesBean">
									  	<div>
									  		<h5>Choose the Subnet to re-map the above rules: <span class="text-danger"> *</span></h5>
									  		<select class="form-control"  id="ID_SUBNET" name="ID_SUBNET">
			                                	<option value="">-</option>
		                                	
			                                	<c:forEach var="subnet" items="${subnets}">
			                                		<c:if test="${(subnet.ID_PARENT==vpc) and (subnet.ID_COMPONENT!=subnetComponent.ID_COMPONENT)}">
			                                			<option value="${subnet.ID_COMPONENT}">${subnet.TX_COMPONENT_NAME} - ${subnet.TX_COMPONENT_KEY}</option>
			                                		</c:if>
			                                	</c:forEach>
									  		</select>
									  		<div>
									  			<input type="text" id="ID" name="ID" value="${subnetComponent.ID_COMPONENT}" style="visibility:hidden">
									  		</div>
									  	</div>
									  	<hr/>
