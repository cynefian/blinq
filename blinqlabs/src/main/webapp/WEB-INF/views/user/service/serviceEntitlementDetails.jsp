	...
	<jsp:body>
	 ...
	 <!-- ============================================================== -->
		...
		<div id="page-wrapper">
			<div class="container">
				<div class="m-t-2 p-2">
	   					<button type="button" class="btn btn-info" onclick="history.back()"><i class="fas fa-arrow-alt-circle-left"></i></button>
						<br/><br/>
				</div>
										                                        ...
										                                        <tbody>
										                                        	<c:forEach var="attach" items="${senAttachList}" varStatus="loop">
											                                            <tr>
											                                                ...
											                                                <%-- <span class="p-2 text-danger"><a href="${contextPath}/admin/deleteSenAttachment?id=${attach.ID}&sen=${entry.ID}" class="text-danger"><i class="fas fa-trash-alt"></i></a></span> --%> 
											                                                </td>
											                                            </tr>
										                                            </c:forEach>
										                                        </tbody>
										                                ...
										                                <ul class="pagination float-right">
                                                                             <c:if test="${currentPage!=1 && currentPage>0}">
									                                        	 <li class="page-item mr-2">
									                                            	<a class="page-link" href="${contextPath}/user/SENdetails?source=ATTACH&id=${entry.ID}&page=${currentPage - 1 }" tabindex="-1">Previous</a>
									                                        	</li>
									                                        </c:if>
									                                    <c:forEach begin="1" end="${totalPages}" varStatus="loop">
									                                    	<c:if test="${currentPage==loop.index}">
									                                    		<li class="page-item active" id="paginationLink${loop.index}"><a class="page-link" href="${contextPath}/user/SENdetails?source=ATTACH&id=${entry.ID}&page=${loop.index}">${loop.index}</a></li>
									                                    	</c:if>
									                                    	<c:if test="${currentPage!=loop.index}">
									                                    		<li class="page-item" id="paginationLink${loop.index}"><a class="page-link" href="${contextPath}/user/SENdetails?source=ATTACH&id=${entry.ID}&page=${loop.index}">${loop.index}</a></li>
									                                    	</c:if>
									                                    </c:forEach>
									                                    	<c:if test="${currentPage!=totalPages}">
									                                        	 <li class="page-item ml-2">
									                                            	<a class="page-link" href="${contextPath}/user/SENdetails?source=ATTACH&id=${entry.ID}&page=${currentPage + 1 }" tabindex="-1">Next</a>
									                                        	</li>
									                                        </c:if>
		...
		<%-- <script src="${pageContext.request.contextPath}/resources/js/blinqlabs/common/serviceEntitlementDetails.js"></script> --%>
		<script>
		
		
		function clearFilter(id){
			var filterForm = document.getElementById("dateFilterForm");
			filterForm.action="${contextPath}/user/SENdetails?id="+id+"&source=ACTIVITY";
		...
		function filterActivity(){
			...
			filterForm.action="${contextPath}/user/SENdetails?id="+senId+"&startDate="+startDate+"&endDate="+endDate+"&source=ACTIVITY";
			filterForm.method = "POST";
			filterForm.submit();
